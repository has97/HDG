...
def get_args():
    ...
    parser.add_argument(
        "--domain_num", type=int, default=4, help="number of domain"
    )
    parser.add_argument(
        "--disttype",
        type=str,
    ...
    parser.add_argument(
        "--mmd_gamma", type=float, default=10, help="MMD, CORAL hyper-param"
    )
...
if __name__ == "__main__":
    ...
    if args.dataset == 'MultiDataSet':
        # eval_name_dict = {"train": [0, 1, 2, 3, 4], "valid": [5, 6, 7, 8, 9], "target": [10, 11, 12, 13]}
        # eval_name_dict = {"train": [0, 1, 2], "valid": [3, 4, 5], "target": [6, 7, 8, 9]}
    ...
    for epoch in range(args.max_epoch):
        for iter_num in tqdm(range(args.steps_per_epoch)):
            ...
            if args.algorithm == "VREx" and algorithm.update_count == args.anneal_iters:
                ...
                opt = opt_cls
            if "Division" in args.algorithm:
                step_vals: dict = algorithm.update(minibatches_device, opt, sch, epoch)
            else:
        ...
        if (epoch in [int(args.max_epoch * 0.9), int(args.max_epoch)]) and (
            not args.schuse
        ):
            print("manually descrease lr")
            if isinstance(opt, list):
        ...
        if (epoch == (args.max_epoch - 1)) or (epoch % args.checkpoint_freq == 0):
            ...
            if acc_record["valid"] >= best_valid_acc:
                ...
                if args.dataset == 'MultiDataSet':
                    ...
                    # target_acc1 = acc_record["target"][0]
                    # target_acc2 = acc_record["target"][1]
                    # target_acc3 = acc_record["target"][2]
                    # target_acc4 = acc_record["target"][3]
                else:
                    target_acc = acc_record["target"]
                # save_checkpoint(
                ...
                # )
                save_checkpoint(
                    f"best_acc_model_and_args_epoch.pkl", algorithm, args
                )
                early_stopping_cnt = 0
            else:
    ...
    if args.dataset == 'MultiDataSet':
        ...
        print("target_acc1: %.4f" % target_acc1)
        print("target_acc2: %.4f" % target_acc2)
        print("target_acc3: %.4f" % target_acc3)
        print("target_acc4: %.4f" % target_acc4)
        ...
        with open(os.path.join(args.output, "done.txt"), "w") as f:
            ...
            f.write("target acc1:%.4f\n" % (target_acc1))
            f.write("target acc2:%.4f\n" % (target_acc2))
            f.write("target acc3:%.4f\n" % (target_acc3))
            f.write("target acc4:%.4f\n" % (target_acc4))
