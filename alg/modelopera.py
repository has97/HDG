# coding=utf-8
import torch
from network import img_network, vision_transformer, efficientnet, mobilenet, swin_transformer
import numpy as np
...
def get_fea(args):
    if args.dataset == "dg5":
        ...
        net = vision_transformer.ViTBase(args.net)
    elif args.net.startswith("eff"):
        net = efficientnet.EffBase(args.net)
    elif args.net.startswith("mob"):
        net = mobilenet.MobBase(args.net)
    elif args.net.startswith("swin"):
        net = swin_transformer.SwinBase(args.net)
    else:
...
def save_pickle(
    ...
    with torch.no_grad():
        ...
        for data in loader:
            x = data[0].to(device).float()  # torch.Size([64, 3, 224, 224])
            y = data[1].to(device).long()  # torch.Size([64])
            feats = network.featurizer(x)  # p means logits. torch.Size([64, known_classes])
            for i in range(len(y)):
                feats_info.append(feats[i].detach().cpu().numpy())
                label_info.append(y[i].item())
                if y[i] >= 6:
                    unknown_info.append(1)
                else:
                    unknown_info.append(0)
        data = [feats_info, label_info, unknown_info]
        path = "./tsne/test_pacs_Coral_0_new.pkl"
        output = open(path, 'wb')
