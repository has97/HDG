...
from utils.util import (
    ...
    get_best_acc_model_and_args_epoch_pkl,
    load_checkpoint,
    print_args,
...
if __name__ == "__main__":
    ...
    if args.dataset == 'MultiDataSet':
        ...
        # eval_name_dict = {"train": [0, 1, 2], "valid": [3, 4, 5], "target": [9]}
        # eval_name_dict = {"train": [0, 3, 4], "valid": [5, 8, 9], "target": [10]}
    else:
    ...
    max_file = get_best_acc_model_and_args_epoch_pkl(args.output, args.algorithm)
    ...
    if args.dataset != 'MultiDataSet':
        ...
        # filename = args.output + f"/eval/threshold_range_list_entropy"
    else:
    ...
    # else:
        # threshold_range_list = modelopera.get_thresholds_entropy(
    ...
    for thd in threshold_range_list:
        if args.algorithm == 'Wild':
            h_scores.append(
                modelopera.h_score_aux(
                    algorithm,
    ...
    if args.dataset != 'MultiDataSet':
        ...
        tmp_PATH = args.output + "/eval_C"
        # tmp_PATH = args.output + f"/eval_C"
        # tmp_PATH = args.output + f"/eval_R"
        # tmp_PATH = args.output + f"/eval_P"
        # tmp_PATH = args.output + f"/eval_K"

    with open(os.path.join(tmp_PATH, "eval_h_score.txt"), "a") as f:
        f.write("done\n")
